...
const DialogUnique2 = ({ tweet }) => {
  const { user } = useUser();
  const { clickCommenterUnique2, addComment } = useTwitterStore();
  const form = useForm();

  const onSubmit = (data)=>{
    addComment(tweet.id,user.imageUrl,tweet.name,data.post)
    clickCommenterUnique2(tweet.id, false)
  }

  return (
    <Dialog open={tweet.dialogCommenter} onOpenChange={(isOpen) => clickCommenterUnique2(tweet.id, isOpen)} >
      <DialogTitle></DialogTitle>
      <DialogContent className="bg-slate-950 text-white h-96 border-0">
        <div>
          <div>
