...
const DialogUnique = ({ tweet }) => {
  const { user } = useUser();
  const { clickCommenterUnique, addComment } = useTwitterStore();
  const form = useForm();

  const onSubmit = (data)=>{
    addComment(tweet.id,user.imageUrl,tweet.name,data.post)
    clickCommenterUnique(tweet.id, false)
  }

  return (
    <Dialog open={tweet.dialogCommenter} onOpenChange={(isOpen) => clickCommenterUnique(tweet.id, isOpen)} >
